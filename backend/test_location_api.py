...
def test_search_by_town(client):
    ...
    data = response.json()
    assert len(data["locations"]) > 0
    assert all("LON" in loc["town"] for loc in data["locations"])
...
def test_search_result_limit(client):
    """Test that search results are limited to 100 entries."""
    # Use a very broad search term that would return many results
